...
import com.clj.jaf.utils.JPrecondition;
...
public class RestClient {
    ...
    private static void get(String url, Map<String, String> customizedHeader, HttpResponseHandler callback, int retryNum) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("RestClient", e.getErrorCode(), e);
            ...
            e.printStackTrace();
            Log.e("RestClient", "get method error!", e);

            if (JStringUtil.isNotBlank(e.getMessage())) {
     ...
     * @throws RestException
     */
    public static void get(final String url, final Map<String, String> customizedHeader,
                           final HttpResponseRetryHandler callback) throws RestException {
        RestClient client;
    ...
    protected RestClient closeOutput() throws IOException {
        if (output == null)
            return this;
        if (multipart)
            output.write(RestConstant.CRLF + "--"
                    + RestConstant.BOUNDARY
                    + "--" + RestConstant.CRLF);
    ...
    public void setRetryNum(int retry_num) {
