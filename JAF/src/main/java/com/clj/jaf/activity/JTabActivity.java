...
public abstract class JTabActivity extends TabActivity implements JIProcessEvent, JITaskListener {
    ...
    private JTabActivity.Status mStatus;
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (JBroadcastByInner.INTENT_ACTION_EVENT.equals(action)) {
                JTabActivity.this.initBroadcastParameterByInner(intent);
                JTabActivity.this.processEventByInner(intent);
            } else if (JBroadcastByProcess.INTENT_ACTION_EVENT.equals(action)) {
                JTabActivity.this.initBroadcastParameterByProcess(intent);
                JTabActivity.this.processEventByProcess(intent);
            }
    ...
    protected void onStart() {
        this.mStatus = JTabActivity.Status.STARTED;
        super.onStart();
    }

    protected void onResume() {
        this.mStatus = JTabActivity.Status.RESUMED;
        super.onResume();
    }

    protected void onPause() {
        this.mStatus = JTabActivity.Status.PAUSED;
        super.onPause();
    }

    protected void onStop() {
        this.mStatus = JTabActivity.Status.STOPPED;
        super.onStop();
    }
    ...
    private void loadDefautLayout() {
        try {
            int layoutResID = JActivityManager.getInstance().getLayoutLoader().getLayoutID(this.mLayouName);
    ...
    public JTabActivity.Status get_status() {
        return this.mStatus;
    }

    public boolean isActivity() {
        return this.mStatus != JTabActivity.Status.DESTROYED
                && this.mStatus != JTabActivity.Status.PAUSED
                && this.mStatus != JTabActivity.Status.STOPPED;
    }
